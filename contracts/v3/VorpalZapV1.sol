...
import {IVorpalPair} from "./interfaces/IVorpalPair.sol";
import {IVorpalRouter02} from "./interfaces/IVorpalRouter02.sol";
import {IWETH} from "./interfaces/IWETH.sol";
import {Babylonian} from "./libraries/Babylonian.sol";

/*
 * @author Inspiration from the work of Zapper and Beefy.
 * Implemented and modified by VorpalSwap teams.
 */
contract VorpalZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped BNB (WBNB)
    IWETH public WBNB;

    // VorpalRouter interface
    IVorpalRouter02 public VorpalRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from VorpalSwap)
    uint256 public constant MINIMUM_AMOUNT = 1000;

    // Maximum reverse zap ratio (100 --> 1%, 1000 --> 0.1%)
    uint256 public maxZapReverseRatio;

    // Address VorpalRouter
    address private VorpalRouterAddress;
     ...
     * @param _VorpalRouter: address of the VorpalRouter
     * @param _maxZapReverseRatio: maximum zap ratio
     */
    constructor(
        address _WBNBAddress,
        address _VorpalRouter,
        uint256 _maxZapReverseRatio
    ) {
        WBNBAddress = _WBNBAddress;
        WBNB = IWETH(_WBNBAddress);
        VorpalRouterAddress = _VorpalRouter;
        VorpalRouter = IVorpalRouter02(_VorpalRouter);
    ...
    {
        address token0 = IVorpalPair(_lpToken).token0();
        address token1 = IVorpalPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = IVorpalPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = VorpalRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = VorpalRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == IVorpalPair(_lpToken).token0() || _token0ToZap == IVorpalPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == IVorpalPair(_lpToken).token0() || _token1ToZap == IVorpalPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == IVorpalPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / IVorpalPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / IVorpalPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = VorpalRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = VorpalRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = IVorpalPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "VorpalRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "VorpalRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        VorpalRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (IERC20(_token).allowance(address(this), VorpalRouterAddress) < 1e24) {
            // Re-approve
            IERC20(_token).safeApprove(VorpalRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = VorpalRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = VorpalRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = VorpalRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = VorpalRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = VorpalRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
