...
contract VestingSale is Ownable {
    ...
    function buyTokens(uint256 amount) external onlyStarted {
        ...
        schedules[msg.sender].amount += vorpalAmount;
        schedules[msg.sender].tokensLeft += vorpalAmount;
        schedules[msg.sender].unlockingStart = block.timestamp + lockPeriod;
        schedules[msg.sender].unlockingEnd =schedules[msg.sender].unlockingStart + vestingPeriod;
        ...
        returns (uint256)
    {
        ...
        if (
            (currentTime - schedule.unlockingStart) * rate > schedule.tokensLeft
        ) {
            return schedule.tokensLeft;
        } else return (currentTime - schedule.unlockingStart) * rate;
    }
}
