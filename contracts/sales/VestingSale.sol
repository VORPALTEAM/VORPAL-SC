...
contract VestingSale is Ownable {
    ...
    ///@param amount - amount of USDC to buy Vorpal tokens
     function buyTokens(uint256 amount, uint256 id) external onlyStarted {
        ...
        schedules[msg.sender].push(VestingSchedule(
            vorpalAmount,
            vorpalAmount,
            unlockingStart,
            unlockingEnd,
...
function withdrawTokens(uint256 amount, uint256 id) external {
    VestingSchedule storage schedule = schedules[msg.sender][id];
    
    if (amount > schedule.amount) {
        revert("Too many tokens requested");
    }

    uint256 unlockedTokens = getUnlockedTokens(schedule);
    uint256 tokensLeft = schedule.tokensLeft;

    // Withdraw the lesser amount between requested 'amount' and 'tokensLeft'
    uint256 withdrawAmount = amount;
    if (unlockedTokens < tokensLeft && amount > tokensLeft - unlockedTokens) {
        withdrawAmount = tokensLeft - unlockedTokens;
    }

    schedule.tokensLeft -= amount;
    vorpal.safeTransfer(address(msg.sender), withdrawAmount);
}
    ...
    function getSchedule(address holder, uint256 id)
    external
    view
    returns (VestingSchedule memory)
{
    return schedules[holder][id];
}
    ...
    returns (uint256)
{
    ...
    function getScheduleById(address holder, uint256 id)
    external
    view
    returns (VestingSchedule memory)
{
    VestingSchedule[] memory holderSchedules = schedules[holder];
    for (uint i = 0; i < holderSchedules.length; i++) {
        if (holderSchedules[i].id == id) {
            return holderSchedules[i];
        }
    }
    revert("No schedule found for this ID");
}
  function getUnlockedTokensInAnyTime(address holder, uint256 id, uint256 currentTime)
    external
    view
    onlyOwner
    returns (uint256)
{
    ...
    if (currentTime < schedule.unlockingStart) {
        return 0;
    } else if (currentTime >= schedule.unlockingEnd) {
        return schedule.amount - schedule.tokensLeft;
    } else {
        uint256 rate = schedule.amount / vestingPeriod;
        uint256 unlockedTokens = (currentTime - schedule.unlockingStart) *
            rate;
        if (unlockedTokens > schedule.amount) {
            return schedule.amount - schedule.tokensLeft;
        } else {
