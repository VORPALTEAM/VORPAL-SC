...
library VorpalLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'889e90fafc7195374644a8bff19f1b2da18c90a21ddd1284279a46fa4f2b229b' // init code hash
            ))));
    }

    function getSwapFee(address factory, address tokenA, address tokenB) internal view returns (uint swapFee) {
        swapFee = IVorpalPair(pairFor(factory, tokenA, tokenB)).swapFee();
    }

    // fetches and sorts the reserves for a pair
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        (address token0,) = sortTokens(tokenA, tokenB);
        (uint reserve0, uint reserve1,) = IVorpalPair(pairFor(factory, tokenA, tokenB)).getReserves();
